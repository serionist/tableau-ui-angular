...
export class DataManager {
    ...
    public async reset(
        dataWindowHeightPx: number,
        dataRowHeightPx: number,
        sort: DataSort[],
    ...
    setScrollPosition(topPx: number) {
        ...
        for (
            let i = firstVisibleBlockId - dataBlockWindow;
            i <= lastVisibleBlockId + dataBlockWindow;
            i++
        ) {
            if (i < 0) {
                continue;
        ...
        this._blocks.update((existing) => {
            ...
            const lastBlock =
                existing.blocks.length === 0
                    ? undefined
                    : existing.blocks[existing.blocks.length - 1];
